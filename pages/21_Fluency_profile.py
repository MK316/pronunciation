...
def score_speech_rate(speech_rate, low, high):
    if low <= speech_rate <= high:
        return 100.0
    dist = min(abs(speech_rate - low), abs(speech_rate - high))
    return max(0.0, 100.0 - dist * 35.0)


def score_articulation_rate(art_rate, low, high):
    adj_low = low + 0.3
    adj_high = high + 1.0
    if adj_low <= art_rate <= adj_high:
        return 100.0
    dist = min(abs(art_rate - adj_low), abs(art_rate - adj_high))
    return max(0.0, 100.0 - dist * 30.0)


def score_pause_ratio(pause_ratio):
    # Lower is better
    # Around 0.10~0.25 is acceptable, larger means interruptions are substantial
    if pause_ratio <= 0.10:
        return 100.0
...
def score_mean_length_of_run(mlr_syllables):
    # Higher is better
    # Rough classroom-oriented scaling
    if mlr_syllables >= 8:
        return 100.0
    elif mlr_syllables >= 6:
        return 85.0
    elif mlr_syllables >= 4:
        return 65.0
    elif mlr_syllables >= 2.5:
        return 45.0
    else:
        return 20.0
...
def analyze_and_score(audio_bytes, syllable_count, target_sps_range, is_student=False):
    try:
        ...
        # number of runs = pauses + 1, but use detected long pauses as chunk boundaries
        n_runs = max(1, long_pauses + 1)
        ...
        total_score = round(np.mean(list(profile_scores.values())))
...
def describe_profile(res):
    sr_score = res["profile_scores"]["Speech rate"]
    ar_score = res["profile_scores"]["Articulation rate"]
    pr_score = res["profile_scores"]["Pause ratio"]
    mlr_score = res["profile_scores"]["Mean length of run"]

    lines = []

    lines.append(
        f"**Speech rate** shows your overall pace across the sentence. "
        f"Your current level is **{fluency_band(sr_score)}**."
    )
    lines.append(
        f"**Articulation rate** shows how fast you speak when you are actually producing speech. "
        f"Your current level is **{fluency_band(ar_score)}**."
    )
    lines.append(
        f"**Pause ratio** shows how much silence interrupts your delivery. "
        f"Your current level is **{fluency_band(pr_score)}**."
    )
    lines.append(
        f"**Mean length of run** shows how long you can continue smoothly before a major break. "
        f"Your current level is **{fluency_band(mlr_score)}**."
    )
...
def overall_feedback_from_profile(res):
    ...
    messages = {
        "Speech rate": "Work on keeping a more stable overall pace from the beginning to the end of the sentence.",
        "Articulation rate": "When you are actually speaking, try to keep the words moving more smoothly and continuously.",
        "Pause ratio": "Too much silence is interrupting the message. Try to reduce unnecessary gaps.",
        "Mean length of run": "Try to produce longer chunks of speech before stopping.",
    }
    ...
    strengths = {
        ...
        "Mean length of run": "You can maintain speech chunks relatively well.",
    }

    return f"{strengths[strongest]} {messages[weakest]}"
...
st.caption("Record 3 short sentences. The app creates a mini fluency profile from four measures.")
...
st.info(
    ...
    f"Recommended overall speech-rate range: {target_sps_range[0]} ~ {target_sps_range[1]} syllables per second"
)
