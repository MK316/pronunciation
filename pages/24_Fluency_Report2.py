...
def start_new_session(user_name, level_name):
...
if not st.session_state.session_started:
    ...
    level_input = st.selectbox("Choose your level", list(LEVELS.keys()), index=0)

    if st.button("Start Session"):
        if not name_input.strip():
            st.error("Please enter the user name first.")
        else:
            start_new_session(name_input, level_input)
...
level_name = st.session_state.selected_level
