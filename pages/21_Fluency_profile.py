...
def draw_wrapped_lines(c, text, x, y, width_chars=78, line_height=5.5 * mm, font_name="Helvetica", font_size=10.5):
...
def build_pdf_report(user_name, start_time, end_time, level_name, avg_score, avg_profile_scores, avg_metrics, band_text):
    ...
    c.setFont("Helvetica-Bold", 12)
    c.drawString(margin_x, y, "How to interpret the result")
    y -= 8 * mm
    ...
    for paragraph in explain_lines:
        y = draw_wrapped_lines(c, paragraph, margin_x, y, width_chars=82)
        y -= 2 * mm

    y -= 3 * mm
    c.setFont("Helvetica-Bold", 12)
    c.drawString(margin_x, y, "Average profile scores")
    y -= 8 * mm
...
st.caption("Record 3 sentences, choose the utterance range once on one waveform, analyze it, and generate a PDF report at the end.")
...
if audio_data:
    ...
    st.success("Recording saved. Adjust the range slider below and then click Analyze and show feedback.")
...
if trial_num in raws:
    ...
    default_start = 0.0
    default_end = float(round(total_duration, 2))
    ...
    st.caption("Move the two handles to choose the first clear speech point and the last clear speech point.")

    selected_range = st.slider(
        "Utterance range (seconds)",
        min_value=0.0,
        max_value=float(round(total_duration, 2)),
        value=(float(round(default_start, 2)), float(round(default_end, 2))),
        step=0.01,
        key=f"range_slider_{level_name}_{trial_num}",
    )
    ...
    st.plotly_chart(
        plot_waveform_with_selection(
            ...
            selected_start,
            selected_end,
            title="Recorded waveform with selected utterance range",
        ),
...
if trial_num < 2:
    if trial_num in level_results:
        ...
        st.caption("Record the sentence, select the range, and analyze it first.")
